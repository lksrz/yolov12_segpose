...
from ultralytics.nn.tasks import DetectionModel, SegmentationModel, SegmentPoseModel, WorldModel
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        elif model.task == "segment_pose":
            # record keypoint shape for combined task as well
            self.metadata["kpt_shape"] = getattr(model.model[-1], "kpt_shape", None)
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        output_names = ["output0", "output1"] if isinstance(self.model, (SegmentationModel, SegmentPoseModel)) else ["output0"]
        dynamic = self.args.dynamic
        if dynamic:
            dynamic = {"images": {0: "batch", 2: "height", 3: "width"}}  # shape(1,3,640,640)
            if isinstance(self.model, (SegmentationModel, SegmentPoseModel)):
